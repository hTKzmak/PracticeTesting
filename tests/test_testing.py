...
class BaseRequest:
    ...
    def _request(self, url, request_type, data=None, expected_error=False):
        ...
        # log part (нужно записать все логи в файл, а не выводить в cmd)
...
# POST_REQUEST(urlUsers, dataUser)


# V2 (не работает):
# new_user = {'name': 'Ryden'}
# user_id = base_request.post('users', 3, new_user)
# pprint.pprint(user_id)


# -----------------PUT-----------------

dataToUpdUser = {'name': "Alan", 'age': 32}
...
# PUT_REQUEST('users', 4, dataToUpdUser)
...
@allure.story('Провека на то, что новый пользователь добавился в db.json')
...
@allure.story('Проверка на то, действительно ли всего 3 постов в posts')
def test_value():
...
@allure.story('Проверка удаления пользователя с db.json')
def test_deletedData():

    # Удаление пользователя с именем Amelie
    # DELETE_REQUEST('users', 2)
...
@allure.story('Проверка получения названия конкретной студии')
...
@allure.story('Проверка на то, действительно ли кол-во данных design_course меньше 5')
def test_value():
